...
{
    for(int i=n-1; i>=0; --i){
        int dost_czas = przedz[i].kon; // dostepny czas - czas pomiedzy aktualna czynnoscia a poprzednia (jesli istnieje)
        if(i > 0)
        ...
        while(dost_czas != 0 && !kandydaci.empty()){
            ...
            if(it->first <= dost_czas){ // jesli najkrotszy miesci sie w calosci
                dost_czas -= it->first;
                wynik.push_back(przedz[przedz[it->second].numer]); // wrzucenie do vectora zawierajacego wszystkie wynikowe przedzialu o numerze przedz[i].numer
            }
            else{ // jesli tylko czesc wchodzi
                kandydaci.insert(make_pair(it->first - dost_czas, it->second)); // wrzucanie najkrotszego pomniejszonego o dostepny czas
                dost_czas = 0;
            }
