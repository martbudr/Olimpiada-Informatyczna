...
int main(void)
{
    ...
    while(!Q.empty()){
        ...
        if(zlacz[curr.second] <= curr.second && ost <= curr.second){ // jesli dany nie zostal jeszcze zlaczony z nastepnym
            ...
            while(true){
                if(ost == n) break;
                while(!(zlacz[ost] <= ost && zlacz[zlacz[ost]] == ost)){ /// dodane
                ...
                if(miej_next.first * 1LL * miej_ost.second >=
                   dlu[0] * 1LL * miej_ost.second * miej_next.second + miej_ost.first * 1LL * miej_next.second) wynik++;
