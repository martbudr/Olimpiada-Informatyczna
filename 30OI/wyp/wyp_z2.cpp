...
pair<LL,LL> calc_mom(int i, int j) // tylko jesli pierwszy jest szybszy od drugiego (jesli ich predkosci sa rowne to to nie zadziala)
{
    ...
    return make_pair(mia[i] * 1LL * mia[j] * (pol[j] - dlu[j] - pol[i]),
                     mia[j] * 1LL * lic[i] - mia[i] * 1LL * lic[j]);
}

pair<LL,LL> calc_pier_mom(int i) // liczenie momentu wyprzedzenia i-tego przez pierwszy
{
    return make_pair(mia[i] * 1LL * mia[0] * (dlu[0] + pol[i]),
                     lic[0] * 1LL * mia[i] - mia[0] * 1LL * lic[i]);
}
...
int main(void)
{
    ...
    while(!Q.empty()){
        ...
        if(zlacz[curr.second] <= curr.second && ost <= curr.second){ // jesli dany nie zostal jeszcze zlaczony z nastepnym
            ...
            while(true){
                if(ost == n) break;
                while(zlacz[ost] > ost){ /// dodane
                    if(zlacz[ost] != INT_MAX) ost = zlacz[ost];
                    else ost++;
                }
                ...
                //if(zlacz[ost] > ost || ost_mom.first * 1LL * curr.first.second > ost_mom.second * 1LL * curr.first.first) break; // jesli jest juz zlaczony lub Bajtazar nie zdazy wyprzedzic kolejnego

                pair<LL,LL> miej_ost, miej_next;// miejsce dla pierwszego i kolejnego
                /*miej_ost = make_pair(ost_mom.first * 1LL * lic[ost] + pol[ost] * 1LL * ost_mom.second * mia[ost],
                                     ost_mom.second * 1LL * mia[ost]);*/
                miej_ost = make_pair(pol[ost] * 1LL * ost_mom.second + ost_mom.first * lic[ost] / mia[ost],
                                     ost_mom.second);
                ...
                if(miej_next.first * 1LL * miej_ost.second - miej_ost.first * 1LL * miej_next.second >=
                   dlu[0] * 1LL * miej_ost.second * miej_next.second) wynik++;
