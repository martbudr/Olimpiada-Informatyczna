    ...
    while(!Q.empty()){
        ...
        if(zlacz[curr.second] <= curr.second && ost <= curr.second){ // jesli dany nie zostal jeszcze zlaczony z nastepnym
            ...
            while(true){
                ...
                if(miej_next.first - miej_ost.first * 1LL * miej_next.second >= dlu[0] * 1LL * miej_ost.second * miej_next.second) wynik++;

                // ost_mom.second * 1LL * mia[zlacz[ost+1]] * (pol[zlacz[ost+1]] - dlu[zlacz[ost+1]]) + ost_mom.first * 1LL * lic[zlacz[ost+1]]
                // - mia[zlacz[ost+1]] * (pol[ost] * 1LL * ost_mom.second + ost_mom.first / mia[ost] * 1LL * lic[ost])
                // >= dlu[0] * 1LL * ost_mom.second * mia[zlacz[ost+1]]
